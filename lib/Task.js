...
class Task {
  ...
  execute(job_specs = undefined) {
    ...
    let job = new Job(this, job_specs, this.stack, `Processing ${job_specs.title}`);
  ...
  async configure(registry_auth, {plan, ...job_specs} = {}, stack = {}) {
    this.stop();

    delete this.err;
    try {
      await registry_auth.get_image_manifest(job_specs.image);
    } catch(err) {
      log.error('Failed to get the image manifest', err);
      log.error(`Invalid image %s, disabling task %s`, job_specs.image, this.name);
      plan = false;
      this.err = String(err);
    }
  ...
  async start() {
    try {
      ...
      do {
        ...
        if(!canceled) {
          let job = new Job(this, this.job_specs, this.stack, `Scheduled run for ${this.job_specs.title}`);
  ...
  async _execute(job) {
    ...
    try {
      ...
      await this.executor(this, job.specs, job.stack);
      this.success_count++;
  ...
  export_status() {
    let {name, current_job, previous_job, next, plan, state, success_count, failure_count, total_duration, job_specs, err} = this;

    return {err, name, next, plan : format(plan), state, success_count, failure_count, total_duration,
