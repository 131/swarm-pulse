  ...
  export_metrics() {
    ...
    for(let [task_guid, {job_specs, previous_job, success_count, failure_count, total_duration}] of Object.entries(this.tasks)) {
      ...
      if(previous_job) {
        ...
        metrics.push(`task_last_tick_ms{docker_stack="${this.STACK_NAME}",task_guid="${task_guid}",task_image="${task_image}"} ${Number(last_update)}`);
        metrics.push(`task_last_duration_ms{docker_stack="${this.STACK_NAME}",task_guid="${task_guid}",task_image="${task_image}"} ${Number(last_update - initial_update)}`);
        metrics.push(`task_total_duration_ms{docker_stack="${this.STACK_NAME}",task_guid="${task_guid}",task_image="${task_image}"} ${Number(total_duration)}`);
      }
      metrics.push(`task_tick_count{docker_stack="${this.STACK_NAME}",task_guid="${task_guid}",task_image="${task_image}"} ${Number(success_count)}`);
      metrics.push(`task_error_count{docker_stack="${this.STACK_NAME}",task_guid="${task_guid}",task_image="${task_image}"} ${Number(failure_count)}`);
    }
