...
class Pulse {
  ...
  async _executor({name : task_name}, job_specs) {
    ...
    return await this.stack_sdk.service_exec(service_payload, credentials);
  }
  ...
  async execute(task_name, job_specs = undefined) {
    ...
    if(job_specs === undefined)
      job_specs = task.job_specs;

    return await task.execute(job_specs);
  }
  ...
  export_metrics() {
    ...
    for(let [task_guid, {previous_job, success_count, failure_count}] of Object.entries(this.tasks)) {
      if(previous_job) {
        let {initial_update, last_update} = previous_job;
        metrics.push(`task_last_tick_ms{task_guid="${task_guid}"} ${Number(last_update)}`);
        metrics.push(`task_last_duration_ms{task_guid="${task_guid}"} ${last_update - initial_update}`);
      }
      metrics.push(`task_tick_count{task_guid="${task_guid}"} ${Number(success_count)}`);
      metrics.push(`task_error_count{task_guid="${task_guid}"} ${Number(failure_count)}`);
    }
