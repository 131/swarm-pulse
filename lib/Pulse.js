  ...
  export_metrics() {
    ...
    for(let [task_guid, {job_specs, previous_job, success_count, failure_count, total_duration}] of Object.entries(this.tasks)) {
      if(previous_job) {
        ...
        metrics.push(`task_last_duration_ms{task_guid="${task_guid}"} ${Number(last_update - initial_update)}`);
        metrics.push(`task_total_duration_ms{task_guid="${task_guid}"} ${Number(total_duration)}`);
        metrics.push(`task_last_image{task_guid="${task_guid}"} ${job_specs.image}`);
      }
