...
class Pulse {
  ...
  constructor(config_path = (process.env['PULSE_CONFIG_PATH'])) {
    ...
    for(let [name, task_specs] of Object.entries(tasks)) {
      ...
      if(task)
        task.configure(this.registry_auth, task_specs).then(task.start.bind(task));
    }
  ...
  async _executor({name : task_name}, job_specs, job_stack) {
    const service_payload = await this.stack_sdk.compose_service(task_name, job_specs, job_stack, this.tasks_ns);
  ...
  async check_tasks_images() {
    ...
    for(const image of images_list) {
      ...
      try {
        ...
        for(let [task_name, task] of Object.entries(this.tasks)) {
          if(image === task.job_specs.image) {
            log.error(`Invalid image %s, disabling task %s`, image, task_name);
            task.plan = false;
            task.err = String(err);
          }
        }
